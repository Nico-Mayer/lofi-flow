<script lang="ts">
	import { clamp, inlineSvg, ytPlayerStateCode } from '$lib/utils';
	import {
		activeRadio,
		dailyRadios,
		favorites,
...
<main>
	...
	<section class="flex items-center">
		<Button
			bind:ref={playBtn}
			aria-label={playerState.value === ytPlayerStateCode('playing') ? 'Pause video' : 'Play video'}
			onclick={onPlayPause}
		>
	...
	<section class="mt-2">
		<Button class="!py-0" onclick={openRadioList} tooltip="< Change Radio" tooltipPosition="right">
			{#if playerState.value === ytPlayerStateCode('playing')}
				...
				>
			{:else if playerState.value === ytPlayerStateCode('playing')}
