<script lang="ts">
	import { activeRadioData, playerState, radioListOpen, volume } from './store.svelte';
	type Props = {
		player: YT.Player;
	};
...
<main>
	...
	<section>
		<button class="btn" onclick={openRadioList}>
			{#if playerState.value === YT.PlayerState.BUFFERING}
				<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"
			...
			{:else}
				<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"
					...
					></svg
				>
				<span>{activeRadioData.value.title}</span>
			{/if}
		</button>
	</section>
</main>
